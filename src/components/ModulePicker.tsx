...
const ModuleCard: React.FC<{ mod: ModuleConfig; onClick: () => void }> = ({ mod, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`group flex items-center gap-3 p-4 bg-zinc-900 border border-zinc-800 hover:border-${color}-500/50 hover:bg-zinc-800 rounded-xl text-left transition w-full`}
    >
      <div className={`w-10 h-10 shrink-0 rounded-lg bg-${color}-900/40 border border-${color}-700 flex items-center justify-center font-black text-${color}-300 text-sm`}>
        {mod.id}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <h3 className="font-bold text-zinc-100 text-sm leading-tight">{mod.title}</h3>
          {status === 'complete' && <Check size={13} className="text-emerald-400 shrink-0" />}
        </div>
        ...
        <div className="flex items-center gap-3 mt-1 text-[10px] text-zinc-600">
          ...
          {progress?.quiz_score !== undefined && <span className="text-emerald-400">Quiz: {progress.quiz_score}/{mod.summative_quiz.length}</span>}
          {progress?.primer_score !== undefined && status !== 'complete' && <span className="text-sky-400">Primer: {progress.primer_score}/3</span>}
        </div>
      </div>
      <ChevronRight size={16} className="text-zinc-600 group-hover:text-zinc-300 shrink-0" />
    </button>
  );
};
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <header className="mb-8 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-black tracking-tight text-zinc-100 mb-1">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-zinc-900 border border-zinc-700 hover:border-sky-500 hover:bg-zinc-800 rounded-lg text-sm font-bold text-zinc-300 hover:text-white transition"
          >
          ...
          return (
            <section key={track} className="mb-8">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className={`text-[11px] font-black uppercase tracking-widest text-${color}-300`}>{track}</h2>
                <div className={`flex-1 border-t border-${color}-900/40`} />
                <span className="text-[10px] text-zinc-600">{mods.length} module{mods.length === 1 ? '' : 's'}</span>
              </div>
